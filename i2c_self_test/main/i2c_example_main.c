...
uint8_t display_send_u8(uint8_t byte);
void byte_to_row(uint8_t x, uint8_t y, uint8_t byte, uint8_t color);
void print_char8(uint8_t x, uint8_t y, char thisChar, uint8_t color);
void print_char16(uint8_t x, uint8_t y, char thisChar, uint8_t color);
void print_char32(uint8_t x, uint8_t y, char thisChar, uint8_t color);
...
/* Отпраить данные, размером buffsize */
uint8_t display_send_data(uint8_t *buffer, size_t buff_size)
{
    uint8_t ret = 0;
...
/* Отпрвить 1 байт данных */
uint8_t display_send_u8(uint8_t byte)
{
    uint8_t ret = 0;

    ret = i2c_master_write_u8(I2C_NUM_0, byte);
...
int display_init(void)
{
    ...
    ret += display_wrcom(0xbe); //Set value Vcom
...
void ssd1327_full_write_buff(void)
{
    ...
    display_send_data(SSD1327_Buffer, sizeof(SSD1327_Buffer));
...
void set_pix_on_buf(uint8_t x, uint8_t y, uint8_t color, bool display)
{
  ...
  //  set_coord(x / 2, y, x / 2, y);
    ...
    /* Если хотим отправлять каждый пиксель, но медленно*/
    //display_send_u8(SSD1327_Buffer[address]);
}
...
void byte_to_row(uint8_t x, uint8_t y, uint8_t byte, uint8_t color)
{
    ...
    {
        if (byte & (1 << i))
            set_pix_on_buf(x, y + i, color, true);
        else
            set_pix_on_buf(x, y + i, 0x00, true);
    }
}
...
void print_char8(uint8_t x, uint8_t y, char thisChar, uint8_t color)
{
    ...
    {
        byte_to_row(x + i, y, font8x8_basic[(unsigned char)thisChar][i], color);
    }
}
...
void print_char16(uint8_t x, uint8_t y, char thisChar, uint8_t color)
{
    ...
    {
        byte_to_row(x + row, y, font16x16[(unsigned char)thisChar][row * 2], color);
        byte_to_row(x + row, y + 8, font16x16[(unsigned char)thisChar][(row * 2) + 1], color);
    }
}
...
void print_char32(uint8_t x, uint8_t y, char thisChar, uint8_t color)
{
    ...
    {
        byte_to_row(x + row, y, font16x32[(unsigned char)thisChar][row * 2], color);
        byte_to_row(x + row, y + 8, font16x32[(unsigned char)thisChar][(row * 2) + 1], color);
    }
}
...
void print_string(uint8_t x, uint8_t y, char *str, uint8_t color)
{
    ...
    for (; *str != '\0'; str++)
    {
        print_char8(y, x + offset, *str, color);
        offset += 8;
    }
}
...
void print_string16(uint8_t x, uint8_t y, char *str, uint8_t color)
{
    ...
    for (; *str != '\0'; str++)
    {
        print_char16(y, x + offset, *str, color);
        offset += 10;
    }
}

void print_string32(uint8_t x, uint8_t y, char *str, uint8_t color)
{
    uint16_t offset = 0;

    for (; *str != '\0'; str++)
    {
        print_char32(y, x + offset, *str, color);
        offset += 16;
    }
}
...
static void i2c_test_task(void *arg)
{
    ...
    while (1)
    {
        ...
        print_string(0, 0, "TEST I2C", White);
        print_string(0, 10, "Some text", White);
        print_string(10, 20, "offset 10x", White);
        print_string(5, 30, "offset 5x", White);
        print_string(15, 40, "offset 15x", White);
        print_string(0, 50, "ABC", White);
        print_string16(0, 60, "TEST 16x", White);
        print_string32(0, 80, "TEST 32X", White);
        cnt++;
        sprintf(str_out, "cnt = %d\n", cnt);
        print_string(0, 110, str_out, White);
        ssd1327_full_write_buff();
    }
